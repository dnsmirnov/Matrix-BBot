...
from bbot import BBError
from bbot.plugins import CommandNotFoundError
from bbot.webhook import NebHookServer
...
class Engine(object):
    ...
    def parse_membership(self, event):
        ...
        if (event["state_key"] == self.config.username
                and event["content"]["membership"] == "invite"):
            username = event["username"]
            if username in self.config.admins:
    ...
    def parse_msg(self, event):
        ...
        if (event["username"] == self.config.username or 
